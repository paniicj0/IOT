...
def dms_callback(pressed, code, on_value=None):
    t = time.localtime()
    print("=" * 20)
...
def run_dms(settings, threads, stop_event, on_value=None):
    delay = 2
    if settings["simulated"]:
        print("Starting DMS simulator")
        th = threading.Thread(
            target=run_dms_simulator,
            args=(delay, lambda pressed, code: dms_callback(pressed, code, on_value), stop_event)
        )
        th.start()
        threads.append(th)
        ...
        th = threading.Thread(
            target=run_dms_loop,
            args=(sensor, delay, lambda pressed, code: dms_callback(pressed, code, on_value), stop_event)
        )
        th.start()
        threads.append(th)
